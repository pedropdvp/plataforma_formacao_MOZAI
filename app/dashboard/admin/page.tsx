...
import { 
  ...
  ArrowUpRight, Plus, Mail, User, ShieldAlert, Globe, Edit2, Trash2, Activity
} from "lucide-react";
...
export default function AdminSettingsPage() {
  ...
  // Estados de Backups da Base de Dados
      ...
      {isGlobalAdmin && (
        <div className="flex border-b border-slate-900 gap-6">
          ...
          <button
            ...
            className={`pb-3 text-xs font-bold transition-all relative ${
              activeTab === "backups" ? "text-indigo-400 font-extrabold" : "text-slate-500 hover:text-slate-300"
            }`}
          >
            Backups
            {activeTab === "backups" && (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 max-w-xl">
                ...
                partilham a mesma base de dados, por isso qualquer backup cobre sempre os dois.
              </p>
            </div>
            <button
              onClick={handleCreateBackup}
              disabled={creatingBackup}
              className="shrink-0 h-10 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-bold text-white flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
            >
